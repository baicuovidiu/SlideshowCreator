...
MOTOR2_API int motor2_nvenc_submit(void* session,void* renderTarget,std::int64_t pts100ns,std::uint64_t* submissionId){
    ...
    auto st=s->api.nvEncEncodePicture(s->encoder,&pp); if(st!=NV_ENC_SUCCESS){s->api.nvEncDestroyBitstreamBuffer(s->encoder,bb.bitstreamBuffer);s->api.nvEncUnmapInputResource(s->encoder,mr.mappedResource);s->api.nvEncUnregisterResource(s->encoder,rr.registeredResource);return st==NV_ENC_ERR_NEED_MORE_INPUT?HRESULT_FROM_WIN32(ERROR_RETRY):E_FAIL;}
