...
public sealed class NativeNvencSession : INvencNativeSession
{
    ...
    public ValueTask OpenAsync(EncoderSettings settings,CancellationToken ct)
    {
        ...
        var ctx=_device.NativeContextForInterop;
    ...
    public ValueTask<ulong> SubmitD3D12SurfaceAsync(object nativeSurface,TimeSpan pts,CancellationToken ct)
    {
        ...
        var hr=Motor2Native.NvencSubmit(_session,h,pts.Ticks,out var id); if(hr<0)Marshal.ThrowExceptionForHR(hr); return ValueTask.FromResult(id);
    }
    ...
    public ValueTask DisposeAsync(){if(_session!=0)Motor2Native.NvencClose(_session);_session=0;return ValueTask.CompletedTask;}
}
